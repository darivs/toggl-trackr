...
2. Copy `.env.example` to `.env` and adjust values:
   ...
   - `FRONTEND_ORIGIN`: set to your site (e.g., `https://toggl-trackr.ouder.space`); comma-separate to allow local + prod.
   - `COOKIE_DOMAIN`: set to `.ouder.space` (or leave empty locally) for shared auth cookies.
